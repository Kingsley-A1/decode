...
import { moderateReview } from "@/server/admin/operations";
import { adminReviewModerationRequestSchema } from "@/server/admin/schemas";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const input = adminReviewModerationRequestSchema.parse(body);
    ...
    return apiSuccess({ data: { review }, requestId });
  } catch (error) {
    return adminApiError({
      error,
      requestId,
      fallbackCode: "ADMIN_REVIEW_UPDATE_FAILED",
      fallbackMessage: "Could not update this review.",
    });
  }
}
