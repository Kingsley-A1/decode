...
const examples: readonly ExampleDoc[] = [
  {
    ...
    code: `curl -X POST https://decode.com.ng/api/qr-codes \\
  ...
  },
  {
    ...
    description:
      "Use this before opening URLs that came from QR scans, contact forms, or user-generated content.",
    code: `curl -X POST https://decode.com.ng/api/links/verify \\
  -H "Content-Type: application/json" \\
  -H "x-request-id: partner-check-001" \\
  -d '{
  ...
  },
  {
    ...
    code: `await fetch("/api/qr-codes/qr_123/render", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  credentials: "include",
...
const result = await response.json();`,
  },
  {
    ...
    code: `const presign = await fetch("/api/assets/presign", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  credentials: "include",
  body: JSON.stringify({
...
// Upload directly to the returned URL, then:
await fetch("/api/assets/asset_123/confirm", {
  method: "POST",
  credentials: "include"
});`,
  },
...
export default function ApiDocsPage() {
  ...
  return (
    <PageShell
      ...
      description="A practical integration reference for Decode QR codes, dynamic redirects, landing pages, assets, link verification, scan decoding, reviews, and admin operations."
      actions={
        <Link
          href="/docs"
    ...
    >
      <section className="grid gap-4 lg:grid-cols-3" aria-label="API summary">
        <SummaryTile
          icon={Code2}
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
        <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2">
            <TerminalSquare className="h-5 w-5 text-sky-700" aria-hidden="true" />
            <h2 className="text-xl font-semibold text-slate-950">
              Standard response shape
            </h2>
          </div>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Decode APIs use one JSON envelope so client code can handle success,
            validation, and server errors consistently.
          </p>
      ...
      <section aria-labelledby="endpoint-reference-title" className="space-y-5">
      ...
      <section aria-labelledby="examples-title" className="space-y-5">
        <div>
          <h2 id="examples-title" className="text-2xl font-semibold text-slate-950">
            Integration examples
          </h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">
            These examples show the shape third-party products should mirror
            when connecting to Decode.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              ...
              server-to-server partner access, add scoped API keys, rate limits,
              audit events, and per-workspace permissions before exposing saved
              QR or asset write endpoints outside Decode.
            </p>
          </div>
          <Link
            href="/support"
